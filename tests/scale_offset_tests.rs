...
fn bitfield_scale_decode() {
    ...
    let buf = [20u8, 0];
    let s = BitfieldScale::unpack(&buf);
...
fn bitfield_scale_roundtrip() {
    for raw in [0u8, 1, 10, 100, 200, 255] {
        let buf = [raw, 0u8];
        let decoded = BitfieldScale::unpack(&buf);
        let reencoded = decoded.pack();
...
fn bitfield_scale_does_not_bleed_into_spare() {
    ...
    let d = BitfieldScale::unpack(&s.pack());
    assert_eq!(d.spare, 0);
}
...
fn bitfield_offset_decode() {
    ...
    let buf = [5u8, 0];
    let s = BitfieldOffset::unpack(&buf);
...
fn bitfield_offset_roundtrip() {
    for raw in [0u8, 1, 50, 127, 200, 255] {
        let buf = [raw, 0u8];
        let decoded = BitfieldOffset::unpack(&buf);
        let reencoded = decoded.pack();
        assert_eq!(reencoded[0], raw, "roundtrip failed for raw={raw}");
    }
}
...
fn bitfield_scale_offset_decode() {
    ...
    let buf = [6u8, 0];
    let s = BitfieldScaleAndOffset::unpack(&buf);
...
fn bitfield_scale_offset_roundtrip() {
    for raw in [0u8, 6, 50, 100, 200, 235] {
        let buf = [raw, 0u8];
        let decoded = BitfieldScaleAndOffset::unpack(&buf);
        let reencoded = decoded.pack();
        assert_eq!(reencoded[0], raw, "roundtrip failed for raw={raw}");
    }
}
...
fn scalar_scale_encode() {
    ...
    let wire = u16::from_le_bytes(s.pack().try_into().unwrap());
...
fn scalar_scale_roundtrip() {
    for raw in [0u16, 1, 50, 100, 500, 1000, u16::MAX / 2] {
        ...
        let decoded = ScalarScale::unpack(&buf);
        let reencoded = u16::from_le_bytes(decoded.pack().try_into().unwrap());
...
fn scalar_scale_zero() {
    ...
    let s = ScalarScale::unpack(&buf);
    assert_eq!(s.value, 0);
    assert_eq!(u16::from_le_bytes(s.pack().try_into().unwrap()), 0);
}
...
fn scalar_offset_encode() {
    ...
    let wire = i16::from_le_bytes(s.pack().try_into().unwrap());
...
fn scalar_offset_roundtrip() {
    for raw in [-1000i16, -500, 0, 100, 500, 1000] {
        let buf = raw.to_le_bytes();
        let decoded = ScalarOffset::unpack(&buf);
        let reencoded = i16::from_le_bytes(decoded.pack().try_into().unwrap());
        assert_eq!(reencoded, raw, "roundtrip failed for raw={raw}");
    }
}
...
fn scalar_scale_offset_decode() {
    ...
    let s = ScalarScaleAndOffset::unpack(&buf);
...
fn scalar_scale_offset_roundtrip() {
    for raw in [-500i16, -100, 0, 100, 500, 1000] {
        let buf = raw.to_le_bytes();
        let decoded = ScalarScaleAndOffset::unpack(&buf);
        let reencoded = i16::from_le_bytes(decoded.pack().try_into().unwrap());
        assert_eq!(reencoded, raw, "roundtrip failed for raw={raw}");
    }
}
